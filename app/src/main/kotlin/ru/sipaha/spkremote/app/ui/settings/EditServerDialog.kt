 ...
 *   [ru.sipaha.spkremote.app.vm.MainViewModel.editServer] and gets back
...
fun EditServerDialog(
    ...
    var host by rememberSaveable(current.id) { mutableStateOf(parsed?.host.orEmpty()) }
    var portText by rememberSaveable(current.id) {
        mutableStateOf(parsed?.port?.toString().orEmpty())
    }
